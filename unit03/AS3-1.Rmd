...
```{r}
sum(D$Arrest) #Answer:15536
mean(D$Arrest)
```

【1.6】 How many observations have a LocationDescription value of ALLEY?
```{r}
sum(D$LocationDescription == "ALLEY") #Answer:2308
...
```{r}
#Answer: "2006-05-21 12:30:00 CST"
TM = as.POSIXct(D$Date, format = "%m/%d/%y %H:%M")
median(TM)
```
...
```{r}
#Answer: No
table(TM > as.POSIXct("2007-01-01"))
```

【3.3】 For what proportion of motor vehicle thefts in 2001 was an arrest made?
```{r}
#Answer: 0.10412
prop.table(table(D$Arrest, format(TM, "%y")), margin = 2)
```
...
```{r}
#Street Parking Lot/Garage(Non.Resid)/Alley/Gas Station/ Driveway-Resdidential
table(D$LocationDescription) %>% sort(TRUE) %>% head(6)
```

【4.2】 How many observations are in Top5?
```{r}
#Answer:177510
Top5 = names(table(D$LocationDescription) %>% sort(TRUE) %>% head(6))
Top5 = Top5[-3]
...
```{r}
#answer: Gas Station
Top5_sub =subset(D, D$LocationDescription %in% Top5)
tapply(Top5_sub$Arrest,Top5_sub$LocationDescription, mean) %>% sort(TRUE)
```

【4.4】 On which day of the week do the most motor vehicle thefts at gas stations happen?
```{r}
#answer: Saturday
table(format(TM[D$LocationDescription == "GAS STATION"], "%u")) %>% sort(T)
...
```{r}
#answer: Saturday
table(format(TM[D$LocationDescription == "DRIVEWAY - RESIDENTIAL"], "%u")) %>% sort(F)
```
