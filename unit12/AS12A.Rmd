...
```{r}
#深藍線：機率函數為f(x) = b0+b1x1+b2x2＝0的邏輯回歸，在其以下的淺藍線則為f(x) = b0+b1x1+b2x2<0的邏輯回歸，以上的淺藍線則為機率f(x)=b0+b1x1+b2x20>的邏輯回歸
```

<span style="font-size:24px"> `r "\U1F5FF"` : </span>
Given any point in the figure above, how can you tell its (predicted) probability approximately?
```{r}
#將點的x1、x2值帶入預測回歸方程式所得即為probablity
```
...
```{r}
#logit：用點分類，將點帶進去回歸線，回歸線輸出值若是>=0，是一類(target)，值<0是另一類(non-target)，也就是0和1
#logit function：logit轉為possibility的函數
#關係：倒數關係
```
